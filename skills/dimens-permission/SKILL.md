...
description: 用于维表智联项目内自定义角色、项目权限、行级策略和单行 ACL 的创建、分配与排查，适合直接落地角色权限方案。
version: 1.0.0
author: 方块智联工作室
...
- ✅ 执行任何权限命令前，先完成认证；认证方式优先参考 `dimens-key-auth`
...
| --- | --- | --- |
| `dimens-key-auth` | 认证、token 获取与失效处理 | 执行任何权限命令前必须先确认 |
