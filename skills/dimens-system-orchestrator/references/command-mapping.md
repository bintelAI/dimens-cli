...
| 上传 SVG 封面/图标 | `dimens-cli upload file --path ./project-cover.svg --team-id TEAM_ID --scene project-cover` | SVG 默认 `250x150px`、淡色背景、轻量动态效果；文件名必须保留 `.svg`，CLI 会按 `image/svg+xml` 上传，上传后再把 URL 写回项目或文档；如果希望同时进入素材管理，再补 `--source material`，该素材库链路优先 CDN 直传并入库 |
...
| 查看字段列表 | `dimens-cli column list --team-id TEAM_ID --project-id PROJECT_ID --sheet-id SHEET_ID` | 建表后立刻检查字段，尤其是自动生成的默认“名称”字段 |
| 移动已有菜单资源到目录 | `dimens-cli sheet move SHEET_OR_REPORT_ID --team-id TEAM_ID --project-id PROJECT_ID --folder-id FOLDER_SHEET_ID` | 已创建的表格/报表等资源不会因目录创建自动归位，必须单独移动；CLI 参数 `--folder-id` 会提交为后端 `parentId` |
...
- 系统搭建落权限时，默认主链是 `role create -> permission create / permission set-resource -> row-policy create -> role assign-user -> myPermissions 回查`
- `role create` 只创建角色记录，不会自动生效；没有 `permission create/update`、`permission set-resource`、`row-policy` 和 `role assign-user` 时，不能说角色权限完成
...
带 `--source material --team-id` 的素材库上传会优先走 CDN：先申请短期上传 token，直传七牛，再完成素材库入库；CDN 未启用或配置不完整时回退本地上传。
...
建表后必须立刻看 `column list`。如果自动生成了默认“名称”字段：

- 能作为主展示字段时，改名为业务字段后复用，例如“客户名称 / 工单标题 / 申请单编号”。
- 不能作为业务字段时，确认没有数据、视图、relation 展示依赖后再按表格章节清理。
- 不要在默认字段旁边再创建一个重复的主展示字段，导致报表和 relation 展示混乱。
...
dimens-cli column create \
  ...
  --label 客户等级 \
  --type select \
  --options '[{"id":"customer_level_a","label":"A 级客户","color":"bg-blue-100 text-blue-700"},{"id":"customer_level_b","label":"B 级客户","color":"bg-slate-100 text-slate-700"},{"id":"customer_level_c","label":"C 级客户","color":"bg-gray-100 text-gray-700"}]'
```

补充：

- `select`、`multiSelect` 等字段在技能链路里必须直接补齐选项值，不要只创空字段
- 稳定枚举优先使用 `select` 或 `multiSelect`：状态、阶段、等级、分类、来源、优先级、风险级别通常不应建成普通 `text`
...
dimens-cli column create \
  ...
  --label 客户等级 \
  --type select \
  --options '[{"id":"customer_level_a","label":"A 级客户","color":"bg-blue-100 text-blue-700"},{"id":"customer_level_b","label":"B 级客户","color":"bg-slate-100 text-slate-700"}]'
```
...
- `--options` 规范写法必须传 JSON 对象数组，以便一次补齐 `id / label / color`；逗号分隔字符串仅是 CLI 兼容旧输入，不作为技能生成规范
- `options[].id` 在同一字段内必须唯一，`options[].label` 不能为空，`options[].color` 只能使用前端 12 色池或 `custom:{\"bg\":\"#xxxxxx\",\"text\":\"#xxxxxx\"}`
...
| 报表 | `row page` 证明数据源有数据，`report preview/query-widget/query` 有可解释结果或明确空数据原因 |
| 画布 | `canvas info` 和保存版本能看到节点、边和版本号 |
| 权限 | 业务角色、表/字段/资源权限、行级策略、用户绑定和 `myPermissions` 能回查到绑定关系 |
