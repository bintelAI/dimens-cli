...
如果输入只是“帮我调 SDK / 接一个 Web 页面”，不要强行做系统蓝图，直接路由到 `dimens-sdk`。
...
- 完整系统搭建需求，验证是否输出表格、文档、报表、业务场景画布和权限边界。
- 审批、流程、自动化需求，验证是否区分业务场景画布和可执行工作流。
- SDK / Web 接入类需求，验证是否转交 `dimens-sdk`，避免系统总控过度拆解。
