...
name: dimens-manager
slug: dimens-manager
description: 用于维表智联项目内业务资源创建、配置、维护和排查，适合处理 Key、团队、项目、表格、富文本字段、JSON 字段、权限、工作流、报表、画布等落地问题。
version: 1.0.3
author: 方块智联工作室
tags: [manager, project, table, richtext, json, permission, workflow, report, canvas, auth, dimens-cli]
---
...
- ✅ 用户提到 `sdk.ai.*`、`response.data`、模型下拉、图片不显示、视频任务轮询、Provider 或 Vite 代理时，属于开发者接入问题；直接路由到 `dimens-sdk/references/ai-examples.md` 与 `dimens-sdk/references/capability-status.md`。
...
- ✅ 当用户消息或当前页面携带维表 URL 时，先解析 URL 上下文。格式 `http://localhost:3000/#/{teamId}/{projectId}/{sheetId}`；例如 `http://localhost:3000/#/TTFFEN/PM394YD/sh_lTyerp8gwW8QSVD1` 表示 `teamId=TTFFEN`、`projectId=PM394YD`、`sheetId=sh_lTyerp8gwW8QSVD1`。用户说“当前页面/当前表/这个表”时优先使用这些 ID。
...
- ✅ 用户涉及 `richtext` 富文本字段或 `json` 字段时，必须读取 `references/table/references/richtext-json-fields.md`；这两类字段必须走专用命令，不能按普通文档或普通单元格处理。
...
| 富文本 / JSON 字段 | `references/table/references/richtext-json-fields.md` | 专用字段创建、读写、存储模式、版本控制与排错 |
...
| SDK / AI 接入故障 | `../dimens-sdk/references/ai-examples.md` | `sdk.ai.*`、响应解包、模型列表、图片结果、视频轮询、Provider、Vite 代理 |
...
4. 再看 `references/team/overview.md`，明确 `teamId / projectId / baseUrl`。
   - 如果用户给了 `/#/{teamId}/{projectId}/{sheetId}` 页面 URL，直接从 URL 得到 `teamId/projectId/sheetId`，只补查缺失或需要校验的部分。
5. 项目容器问题看 `references/project/overview.md`。
6. 数据模型问题看 `references/table/overview.md`。
   - 涉及 `richtext` 或 `json` 字段时继续读取 `references/table/references/richtext-json-fields.md`。
...
- 不要在 `dimens-manager` 内推断 `sdk.ai.*` 的方法签名、`response.data` 层级、前端 Provider 或 Vite 实现；这类问题转到 `dimens-sdk`，manager 只保留 `teamId/projectId`、资源归属和业务验收边界。
...
- 不要在用户已经给出维表页面 URL 时继续笼统追问当前团队、项目或表；先解析 URL，缺哪一段只追问哪一段。
- 不要把人员/部门字段粗暴退化成普通下拉字段。
- 不要用 `doc update` 写富文本字段；富文本正文是 HTML，必须使用 `richtext-field save`。
- 不要用 `row set-cell`、`row create` 或 `row update` 写 JSON 字段；必须区分 inline/extended 后使用 `json-field save`。
- 不要跳过报表预检链直接创建复杂图表。
...
| 用户给了页面 URL 还追问当前表 | 从 `/#/{teamId}/{projectId}/{sheetId}` 解析上下文，例如 `/#/TTFFEN/PM394YD/sh_lTyerp8gwW8QSVD1` |
...
| SDK 调用成功但模型、图片或视频结果为空 | 路由到 `dimens-sdk/references/ai-examples.md`，先核对 SDK 版本和脱敏真实响应，不在 manager 内硬编码返回层级 |
...
- 富文本与 JSON 专用字段的创建、首次保存、读取、二次更新和错误命令拦截。
...
- `references/table/references/richtext-json-fields.md`
