...
- ✅ 这是登录方式扩展，不是新开放平台
- ✅ CLI 首选入口是 `dimens-cli auth api-key-login`；Key 创建、状态、删除、重置等未完整 CLI 化的能力不能伪装成命令能力
...
- ✅ 缺少 `apiKey/apiSecret/baseUrl` 时，先说明缺失项；不能编造示例密钥当成可执行值
- ✅ 在 Windows 终端保存含中文的登录说明、接口样例或返回日志时，必须使用 UTF-8 写入并读回确认
...
- 登录类输出必须包含：使用的 `baseUrl`、是否写入本地 profile、返回是否包含 `token/refreshToken/teamIds`，不要打印完整密钥或完整 token。
- 能力边界输出必须先标注 `已封装 / server-only / 部分对齐`，再给 CLI 或接口案例。
- 如果登录成功但资源访问失败，下一步验证必须转到用户权限和团队项目上下文，而不是重复重置 Key。
- 如果缺少真实密钥或网络环境，必须明确标注“未执行登录”，只给命令模板和排查顺序。
