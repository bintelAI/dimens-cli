...
强制前提：维表智联在线文档使用 TipTap 编辑器。任何 `doc create` / `doc update` 的内容都必须按 TipTap 富文本可编辑结构生成，默认使用 `--format richtext`；不要输出 Markdown 源码、纯文本备注或只有普通段落的简单 HTML。
...
如果 Skill 要帮用户生成项目说明、制度、操作手册、知识页，默认必须按下面结构组织：

1. 首屏标题区：`h1` 主标题 + 1 段简短摘要。
2. 概览背景块：淡色背景卡片，写清目标、范围或核心结论。
3. 状态/分类标签：2-4 个语义稳定的彩色标签。
4. 小节标题：至少 2 个 `h2`，复杂文档继续用 `h3`。
5. 正文与步骤：段落、列表、表格或代码块按内容选择。
6. 提示/风险区：用淡色背景块表达下一步、风险、注意事项。
7. 附件 / 图片 / 相关链接：按需补占位或真实节点。
8. 业务流程图 / 数据流图 / 审批流图：涉及流程时必须补 Mermaid。

不要直接输出一大段没有层次的正文，也不要生成只有黑白文字的单调文档。

### 3.1 演示感文档硬规则

“生动”不是堆颜色，而是让文档像可直接演示的说明页。生成或修改文档时默认遵守：

| 区域 | 强制要求 |
| --- | --- |
| 首屏 | 必须有醒目的 `h1`、摘要段和一个淡色背景概览块 |
| 标题 | 用 `h2/h3` 分层，不要连续堆大段正文 |
| 背景 | 只给摘要、提示、风险、下一步等局部块使用淡色背景 |
| 文字 | 重点词可加粗或局部着色，但正文保持可读，不整段高饱和 |
| 标签 | 状态、分类、角色、优先级用圆角标签表达 |
| 流程 | 涉及业务流、审批流、状态流转时用 Mermaid，不用截图替代 |
| 更新 | `doc update` 要保留原有有效内容，只增强层级、卡片、标签和图表 |

禁止为了“演示感”生成整页深色背景、大面积渐变、闪烁动画、复杂脚本或无法在 TipTap 中继续编辑的结构。
...
生成 `doc create` 或 `doc update` 内容前后，用这张清单自检：

| 检查项 | 通过标准 |
| --- | --- |
| TipTap 格式 | 使用富文本 HTML 或 TipTap 可解析片段，默认 `--format richtext` |
| 标题层级 | 至少有 `h1` 和 2 个 `h2`；复杂文档有 `h3` |
| 首屏表现 | 有摘要段 + 淡色背景概览卡片 |
| 颜色语义 | 至少 2 处有意义颜色表达，且颜色和状态含义一致 |
| 背景块 | 使用局部淡色背景块，不使用整页深色背景 |
| 结构组件 | 至少包含列表、提示块、状态标签中的两类 |
| 流程内容 | 涉及流程、审批、状态流转、系统对接时包含 Mermaid |
| 更新安全 | 修改前先 `doc info`，使用当前 `version`，更新后回读确认 |

任一项不通过时，不要声称文档已经“生动”或“可演示”。
...
```html
<h1 style="font-size:28px;margin-bottom:8px;">工程项目管理平台</h1>
<p style="color:#475569;font-size:15px;">面向项目经理、施工负责人和财务人员的项目进度、成本、质量与文档协同说明。</p>
<div style="background:#eef2ff;color:#3730a3;border-left:4px solid #818cf8;padding:14px 16px;border-radius:10px;margin:14px 0;">
  <strong>一句话定位：</strong>把项目计划、现场执行、成本控制和交付资料集中到同一个维表项目中。
</div>
<p>
  <span style="background:#dbeafe;color:#1d4ed8;padding:3px 10px;border-radius:999px;">项目初始化</span>
  <span style="background:#ecfdf5;color:#047857;padding:3px 10px;border-radius:999px;margin-left:8px;">多角色协作</span>
  <span style="background:#fff7ed;color:#c2410c;padding:3px 10px;border-radius:999px;margin-left:8px;">权限待确认</span>
</p>
<h2>核心交付物</h2>
<ul>
  <li><strong style="color:#2563eb;">表格：</strong>项目台账、施工进度、质量安全、成本付款。</li>
  <li><strong style="color:#7c3aed;">文档：</strong>项目制度、操作说明、交付清单。</li>
  <li><strong style="color:#059669;">报表：</strong>进度概览、成本趋势、风险分布。</li>
</ul>
<div style="background:#fff1f2;color:#be123c;border-left:4px solid #fb7185;padding:12px 14px;border-radius:10px;margin:12px 0;">
  <strong>风险提示：</strong>上线前必须确认角色权限、行级可见范围和报表数据源。
</div>
```

### 9.3 带 Mermaid 的业务流程片段
...
- 不要只生成 `<p>欢迎使用在线文档</p>` 这类占位内容；即使是最小文档，也要有标题、摘要卡片、状态标签和小节
- 不要把 Markdown 原文直接塞进 `--content`；应转换成 TipTap 可编辑的富文本 HTML 或约定片段
