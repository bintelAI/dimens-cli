...
本文档同时作为 AI 工作流节点类型白名单的模板来源。当前模板只允许生成 `start`、`action`、`llm`、`condition`、`end` 五类 `type`；没有模板支撑的未知 workflow node type 不得输出。
...
5. 如果用户要求的节点语义没有对应模板，先改写为已有模板类型并把语义放进 `data`，或写入落地计划，不要新增 `type`。
