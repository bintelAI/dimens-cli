# dimens-manager 报表章节 Recharts 组件生成规范
...
特别注意：统计卡片的真实类型是 `stat`，不是 `statistic`。如果用户说“指标卡、统计卡片、总数卡片、KPI 卡片”，统一映射到 `stat`。
...
- `dataMapping` 至少包含 `nameKey` 和 `valueKey`
- `valueKey` 指向的字段必须是 `number` 或预览阶段可稳定聚合为数字
