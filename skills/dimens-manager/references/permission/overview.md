...
- ✅ 系统搭建或项目初始化时只创建业务角色；不要重复创建平台内置管理员、编辑者、查看者、公开角色等基础角色。
- ✅ `role create` 不等于权限完成；必须继续配置项目/表权限、资源权限、字段可见/只读、行级策略，并绑定用户或明确待绑定对象。
...
| 步骤 | 强制规则 | 验证方式 |
| --- | --- | --- |
| 角色设计 | 只创建业务角色，不重复创建内置管理员、编辑者、查看者、公开角色 | 对照现有角色列表或内置角色说明 |
| 角色创建 | `role create` 后必须拿到 `roleId` | `role info/list` 回查 |
| 表/项目权限 | 为业务角色配置项目级或表级权限 | `permission list` 或对应权限详情回查 |
| 资源权限 | 文档、报表、页面等使用 `permission set-resource` | 回查资源可见/可编辑配置 |
| 字段权限 | 需要列可见/只读时配置 `columnVisibility/columnReadonly` | 回查权限记录和前端权限快照 |
| 行级策略 | “只能看自己/部门/区域”等必须落到 `row-policy` 或 ACL | `row-policy list`，并用行分页验证 |
| 用户绑定 | 业务角色必须绑定用户，或在输出中明确待绑定对象 | `role assign-user` 后回查 role users |
| 生效验证 | 不能只看 CLI success | `myPermissions`、权限快照、缓存刷新和前端重载风险都要说明 |
...
- 系统搭建时不要重复创建“超级管理员 / 系统管理员 / 管理员 / 编辑者 / 查看者 / 公开角色”等平台基础角色；有特殊业务语义时改成业务角色名，例如“销售主管 / 财务审核 / 项目成员”
